    ...
    <div class="white-header">
        <div class="white-bar">
                ...
                <div class="menus">
                    ...
                    <div class="single-default" v-bind:class="{'single-selected': (cat.id==selectedCatId)}"
                        ...
                        <a href="javascript:void(0);" class="white-header-left" style="text-decoration:none;"
                           v-bind:class="{'menu-selected': (cat.id==selectedCatId)}"
                           @click="queryByCategory(cat.id)">{{cat.name}}</a>
                    </div>
                </div>
            </div>
            ...
            <div class="right-part">
                ...
                <a href="javascript:(0);" @click="goAdminCenter()" class="white-header-right"
        ...
        <div class="detail-really">
            ...
            <div class="left-social">
                ...
                <div class="imooc-words">
                    <span class="imooc-num">奇点社区媒体号</span>
                </div>
            ...
            <div class="container-middle">
                <div class="article-wrapper">
                    ...
                    <div class="declare">
                        免责声明：本平台所有内容仅供测试，且文章来自互联网，不代表本网站的观点和立场，如有不妥，请联系后删除。
                    </div>
                </div>
    ...
    var articleList = new Vue({
        ...
        methods: {
            ...
            getCommentCounts() {
                ...
                var articleServerUrl = app.articleServerUrl;
                axios.defaults.withCredentials = true;
                axios.get(articleServerUrl + "/portal/comment/counts?articleId=" + me.articleId)
                    .then(res => {
                        // console.log(JSON.stringify(res.data));
                        if (res.data.status == 200) {
            ...
            getAllComments(page, pageSize) {
                ...
                var articleServerUrl = app.articleServerUrl;
                axios.defaults.withCredentials = true;
                axios.get(articleServerUrl + "/portal/comment/queryPageList?articleId=" + me.articleId +
                    "&page=" + page +
                    "&pageSize=" + pageSize)
                    .then(res => {
                        // console.log(JSON.stringify(res.data));
            ...
            doComment(fatherCommentId) {
                ...
                var commentContent = $('#summernote').summernote('code');
                // 登录后才可以发表评论
                var utoken = app.getCookie("utoken");
                var uid = app.getCookie("uid");
                if (app.isEmpty(uid) || app.isEmpty(utoken)) {
                    // alert("请登录后再发表评论！");
                    this.$notify({
                        title: '警告',
                        message: '请登录后再发表评论！',
                    ...
                    // 调用后端，留言保存
                    this.commentDisplay(fatherCommentId, commentContent);
                }
            },
            // 调用后端，留言保存
            commentDisplay(fatherCommentId, commentContent) {
                ...
                var articleServerUrl = app.articleServerUrl;
                axios.defaults.withCredentials = true;
                axios.post(articleServerUrl + '/portal/comment/addComment',
                    commentReplyBO,
                    {
                        headers: {
                            'headerUserId': uid,
            ...
            getAllCategory() {
                ...
                axios.defaults.withCredentials = true;
                axios.get(adminServerUrl + "/portal/category/queryCategories")
                    .then(res => {
                        // console.log(JSON.stringify(res.data));
